...
# Pytorch version of models for AudioSet: A Large Scale Dataset of Audio Events
...
For more details about AudioSet and the various models they have trained, please
visit the [AudioSet website](http://g.co/audioset) and read their papers:
...
If you use the pre-trained VGGish model in your published research, they ask that
...
They provide a TensorFlow definition of this model, which we call __*VGGish*__, as
...
be at least version 1.0.  They have tested that everything works on Ubuntu and
Windows 10 with Python 3.6.6, Numpy v1.15.4, SciPy v1.1.0, resampy v0.2.1,
Pytorch 1.0.1, Six v1.11.0 and PySoundFile 0.9.0.

VGGish also requires downloading two data files:

* [VGGish model checkpoint](https://storage.googleapis.com/deep_learning_enis/speech_audio_understanding/tf_to_pytorch_vggish_model.pth),
...
* They drop the last group of convolutional and maxpool layers, so they now have
  only four groups of convolution/maxpool layers instead of five.

* Instead of a 1000-wide fully connected layer at the end, they use a 128-wide
...
They provide our own NumPy implementation that produces features that are very
similar to those produced by our internal production code. This results in
embedding outputs that are closely match the embeddings that they have already
...
different underlying libraries and hardware. However, they expect that the
...
They provide a Python implementation of the postprocessing which can be applied to
...
Original authors and reviewers of the code in original package include (in
alphabetical order):
